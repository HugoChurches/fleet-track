...
<form class="max-w-md space-y-3" action="{{route('trips.update', ["trip"=>$trip])}}" method="POST" novalidate>
    ...
    <div class="flex flex-col font-semibold">
        ...
        <input class="w-full rounded border px-2" name="duration_hours" id="duration_hours" placeholder="Duration hours" type="text" required value="{{ old('duration_hours', $trip->duration_hours) }}">
    ...
    <div class="flex flex-col font-semibold">
        ...
        <input class="w-full rounded border px-2" name="destination" id="destination" placeholder="Destination" type="text" required value="{{ old('destination', $trip->destination) }}">
        @error('destination')
        <p class="text-red-400 font-semibold">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="flex flex-col font-semibold">
        ...
        <input class="w-full rounded border px-2" name="due_date" id="due_date" placeholder="Due date" type="date" required value="{{ old('due_date', $trip->due_date) }}">
        @error('due_date')
        <p class="text-red-400 font-semibold">{{ $message }}</p>
        @enderror
    </div>
